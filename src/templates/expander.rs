...
use crate::value::{Builder, IonElement, IonSequence, IonStruct};
...
impl Expander {
    pub fn expand(input: &Element) -> Vec<Element> {
    ...
    fn expand_sequence_elements(input: &Element) -> Vec<Element> {
        let mut expanded_child_elements = vec![];
        for child in input.as_sequence().unwrap().iter() {
            expanded_child_elements.extend(Expander::expand(child).into_iter());
        }
        expanded_child_elements
    }

    // Always returns a new struct
    fn expand_struct_fields(input: &Element) -> Element {
    ...
    fn expand_invocation(input: &Element) -> Vec<Element> {
    ...
    fn empty(_values: &[&Element]) -> Vec<Element> {
        Vec::new()
    }

    // Return a copy of the elements without performing expansion
    fn quote(values: &[&Element]) -> Vec<Element> {
        values.iter()
            .map(|e| (*e).to_owned())
            .collect()
    }
    ...
    fn stream(values: &[&Element]) -> Vec<Element> {
        values.iter()
            .flat_map(|e| Self::expand(e))
            .collect()
    }

    fn inline(values: &[&Element]) -> Vec<Element> {
        values
            .iter()
            .flat_map(|e| {
                let sequence = e
    ...
    fn repeat(values: &[&Element]) -> Vec<Element> {
        if values.is_empty() {
    ...
    fn if_empty(values: &[&Element]) -> Vec<Element> {
        if values.is_empty() {
    ...
    fn annotated(values: &[&Element]) -> Vec<Element> {
        if values.len() != 2 {
    ...
    fn join_into_sequence(values: &[&Element]) -> Sequence {
        let elements: Vec<Element> = values.iter()
            .flat_map(|e| Self::expand(e))
            .collect();
...
mod tests {
    ...
    fn expansion_test(input_ion: &str, expected_ion: &str) {
    ...
    #[test]
    fn plain_old_values() {
        expansion_test("1 2 3", "1 2 3");
    }

    #[test]
    fn stream() {
        expansion_test("1 (:stream 2 3) 4", "1 2 3 4");
        expansion_test("{foo: (:stream 1 2 3) }", "{foo: 1, foo: 2, foo: 3}")
    }

    #[test]
    fn repeat() {
        expansion_test("foo (:repeat 3 bar) baz", "foo bar bar bar baz");
        expansion_test("foo (:repeat 0 bar) baz", "foo baz");
        expansion_test("foo (:repeat 3 (:repeat 2 x)) baz", "foo x x x x x x baz");
    }

    #[test]
    fn inline() {
        expansion_test("1 (:inline [2, 3] (4 5)) 6", "1 2 3 4 5 6");
        expansion_test("{foo: (:inline (1 2 3)) }", "{foo: 1, foo: 2, foo: 3}")
    }

    #[test]
    fn if_empty() {
        expansion_test("foo (:if_empty (:empty) baz) quux", "foo baz quux");
        expansion_test("foo (:if_empty bar baz) quux", "foo bar quux");
        expansion_test("foo (:if_empty (:inline [1, 2]) baz) quux", "foo 1 2 quux");
        expansion_test("foo (:if_empty (:inline []) baz) quux", "foo baz quux");
    }

    #[test]
    fn annotated() {
        expansion_test("(:annotated (:stream foo bar baz) 7)", "foo::bar::baz::7");
        expansion_test("(:annotated (:inline (foo bar baz)) 7)", "foo::bar::baz::7");
    }

    #[test]
    fn string() {
        expansion_test("(:string foo bar baz)", "\"foobarbaz\"");
        expansion_test("(:string 'foo' \"bar\" \"\"\"baz\"\"\")", "\"foobarbaz\"");
        expansion_test("(:string (:inline ('foo' \"bar\" \"\"\"baz\"\"\")))", "\"foobarbaz\"");
    }

    #[test]
    fn symbol() {
        expansion_test("(:symbol foo bar baz)", "foobarbaz");
        expansion_test("(:symbol 'foo' \"bar\" \"\"\"baz\"\"\")", "foobarbaz");
        expansion_test("(:symbol (:inline ('foo' \"bar\" \"\"\"baz\"\"\")))", "foobarbaz");
    }

    #[test]
    fn list() {
        expansion_test("(:list foo bar baz)", "[foo, bar, baz]");
        expansion_test("(:list [])", "[[]]");
        expansion_test("(:list (:inline (1 2 3)))", "[1, 2, 3]");
    }

    #[test]
    fn test_struct() {
        expansion_test("(:struct foo bar baz quux)", "{foo: bar, baz: quux}");
        expansion_test("(:struct foo bar {baz: quux, quuz: gary})", "{foo: bar, baz: quux, quuz: gary}");
    }
}
